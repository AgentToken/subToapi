export default {
    accounts: {
      ...
      openai: {
        ...
        codexFingerprintModeDesc: '多人共享同一 OAuth 账号时，将各用户的设备/会话标识收敛为账号级恒定值，减少上游可见的设备数和会话数。默认关闭（原样透传客户端标识），需要时再显式开启；部分账号开启收敛后出现过额度缩水，请按自己的实测结果选择。smart（会话绑定）按会话记忆首次观测到的安装身份：老会话保持历史身份、新会话绑定账号恒定 installation，只管理 installation 不改写会话/线程标识。',
        codexFingerprintOff: '关闭（透传，默认）',
        codexFingerprintDevice: '仅设备',
        codexFingerprintSession: '设备+会话',
        codexFingerprintFull: '完全收敛',
        codexFingerprintSmart: '会话绑定（smart，推荐）',
