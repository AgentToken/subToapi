...
func (r *stubApiKeyRepo) ListHoneypotKeys(ctx context.Context, limit int) ([]service.APIKey, error) {
	return nil, errors.New("not implemented")
}
...
func TestNewAPIKeyAuthMiddlewareRegistersHoneypotInterceptor(t *testing.T) {
	// 回归：蜜罐拦截器曾被 wire 当作无消费者 provider 裁掉，导致生产拦截静默失效。
	// 认证中间件构造时必须注册拦截器。
	NewAPIKeyAuthMiddleware(nil, nil, nil, &config.Config{})
	if honeypotAuthInterceptor.Load() == nil {
		t.Fatal("honeypot interceptor not registered after NewAPIKeyAuthMiddleware")
	}
}
