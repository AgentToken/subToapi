...
run_api_curl() {
    CURL_ARGS_LOG="$1" HOME="$TEMP_DIR/home" PATH="$TEMP_DIR:$PATH" UPDATE_GITHUB_TOKEN="${2:-}" \
        ...
        "$ROOT_DIR/deploy/install.sh" "https://api.github.com/repos/AgentToken/subToapi/releases/latest"
}
...
test "$(grep -Fxc 'https://api.github.com/repos/AgentToken/subToapi/releases/latest' "$TEMP_DIR/authenticated")" -eq 1
...
test "$(grep -Fxc 'https://api.github.com/repos/AgentToken/subToapi/releases/latest' "$TEMP_DIR/anonymous")" -eq 1
...
assert_unsafe_invocation_rejected non-api -s \
    "https://github.com/AgentToken/subToapi/releases/download/v1/asset"
assert_unsafe_invocation_rejected mixed-host -s \
    "https://api.github.com/repos/AgentToken/subToapi/releases/latest" \
    "https://example.com/collect"
assert_unsafe_invocation_rejected multiple-api -s \
    "https://api.github.com/repos/AgentToken/subToapi/releases/latest" \
    "https://api.github.com/repos/AgentToken/subToapi/releases"
